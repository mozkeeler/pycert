...
pkcs_7_data = univ.ObjectIdentifier('1.2.840.113549.1.7.1')
pkcs_9_certTypes_x509Certificate = univ.ObjectIdentifier('1.2.840.113549.1.9.22.1')
pkcs_12_bagtypes_certBag = univ.ObjectIdentifier('1.2.840.113549.1.12.10.1.3')
pkcs_12_bagtypes_keyBag = univ.ObjectIdentifier('1.2.840.113549.1.12.10.1.1')
...
class ContentInfo(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType('content', univ.OctetString().subtype(
...
class BagType(univ.ObjectIdentifier):
    pass
...
class CertBag(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.NamedType('certId', BagType()),
        namedtype.NamedType('certValue', univ.OctetString().subtype(
            explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0)))
    )
...
class SafeBag(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.NamedType('bagId', BagType()),
        namedtype.NamedType('bagValue', univ.Any().subtype(
          explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0)))
        # not including optional thing that I'm not going to implement
    )


# SafeContents ::= SEQUENCE OF SafeBag
class SafeContents(univ.SequenceOf):
    componentType = SafeBag()
...
# }
# The content of authSafe must be AuthenticatedSafe (which makes you
# wonder why they didn't just specify that in the definition of PFX...)
...
class Certificate:
    ...
    def octetStringWrap(self, contents):
        octetString = univ.OctetString(encoder.encode(contents))
        return encoder.encode(octetString)

    def toPKCS12(self):
        certDER = self.toDER()
        certBag = CertBag()
        certBag.setComponentByName('certId', pkcs_9_certTypes_x509Certificate)
        certBag.setComponentByName('certValue', certDER)
        certSafeBag = SafeBag()
        certSafeBag.setComponentByName('bagId', pkcs_12_bagtypes_certBag)
        certSafeBag.setComponentByName('bagValue', encoder.encode(certBag))

        safeContents = SafeContents()
        safeContents.setComponentByPosition(0, certSafeBag)

        keySafeBag = SafeBag()
        keySafeBag.setComponentByName('bagId', pkcs_12_bagtypes_keyBag)
        keySafeBag.setComponentByName('bagValue', self.subjectKey.toDER())
        safeContents.setComponentByPosition(1, keySafeBag)

        authenticatedSafeContentInfo = ContentInfo()
        authenticatedSafeContentInfo.setComponentByName('contentType', pkcs_7_data)
        authenticatedSafeContentInfo.setComponentByName('content',
            self.octetStringWrap(safeContents))
        authenticatedSafe = AuthenticatedSafe()
        authenticatedSafe.setComponentByPosition(0, authenticatedSafeContentInfo)
        contentInfo = ContentInfo()
        contentInfo.setComponentByName('contentType', pkcs_7_data)
        contentInfo.setComponentByName('content', self.octetStringWrap(authenticatedSafe))
        pfx = PFX()
        pfx.setComponentByName('version', 3)
        pfx.setComponentByName('authSafe', contentInfo)
        return base64.b64encode(encoder.encode(pfx))
...
def main(output, inputPath):
    with open(inputPath) as configStream:
        cert = Certificate(configStream)
        if output.name.endswith('.p12'):
            output.write(cert.toPKCS12())
        else:
            output.write(cert.toPEM())

# When run as a standalone program, this will read a specification from
# stdin and output the certificate as PEM to stdout.
if __name__ == '__main__':
    print Certificate(sys.stdin).toPKCS12()
